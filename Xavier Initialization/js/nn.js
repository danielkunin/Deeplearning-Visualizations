...
function init_params(X) {
	var params = {},
		...
		scalar = [0, 0.01, 1, 100]
		variance = scalar[$('#slider').slider("option", "value")];

	for (var l = 1; l <= L; l++) {
		params['W' + l] = dl.tidy(() => { return dl.randomNormal([layers_dims[l], layers_dims[l-1]]).mul(dl.scalar( variance * layers_dims[l-1] / 2).sqrt()); });
